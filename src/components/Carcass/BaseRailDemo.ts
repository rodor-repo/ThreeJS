...
export class BaseRailDemo {
  ...
  private updatePositioningLabels(): void {
    ...
    this.scene.children = this.scene.children.filter(child => 
      !(child.userData && child.userData.isPositioningLabel)
    );
    ...
    if (this.baseCabinet) {
      ...
      if (baseTop && baseTop.getPositioningInfo) {
        const baseInfo = this.createTextLabel(baseTop.getPositioningInfo(), 0x1E40AF);
    ...
    if (this.topCabinet) {
      ...
      if (topTop && topTop.getPositioningInfo) {
        const topInfo = this.createTextLabel(topTop.getPositioningInfo(), 0x059669);
