...
export class MaterialDemo {
  ...
  static createWithSpecificMaterial(materialId: string = 'premium-wood') {
    ...
    if (material) {
      const carcass = CarcassAssembly.createBaseCabinet(dimensions, { material });
   ...
   */
  static updateMaterialOnCarcass(carcass: CarcassAssembly, newMaterialId: string) {
  ...
  static listAvailableMaterials() {
    ...
    materialIds.forEach((id, index) => {
      ...
      console.log(`  ${id}: ${material.getColour()} (${material.getPanelThickness()}mm)`);
    });
  ...
  static createCustomMaterial() {
    ...
    const dimensions = { width: 600, height: 800, depth: 500 };
    const carcass = CarcassAssembly.createBaseCabinet(dimensions, { material: customMaterial });
