...
export class DrawerCalculator {
   ...
   */
  static calculateDefaultDrawerHeights(carcassHeight: number, drawerQuantity: number): number[] {
  ...
  static calculateHeightBalance(
    ...
    const totalDrawerHeight = drawerHeights.reduce((sum, height) => sum + height, 0);
  ...
  static validateDrawerConfiguration(
    ...
    if (totalHeight > carcassHeight) {
      errors.push(`Total drawer height (${totalHeight}mm) exceeds carcass height (${carcassHeight}mm)`);
    }
