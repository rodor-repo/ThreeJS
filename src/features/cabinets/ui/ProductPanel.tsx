...
const DynamicPanelWithQuery: React.FC<LocalProductPanelProps> = ({ isVisible, onClose, selectedCabinet, onDimensionsChange, onMaterialChange, onOverhangDoorToggle, onShelfCountChange, onDrawerHeightChange, onDrawerQuantityChange, cabinets }) => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const cabinet of cabinets) {
        const productId = cabinet.carcass.productId
        if (productId !== data.product.productId) continue
...
const DynamicPanel: React.FC<DynamicPanelProps> = ({ isVisible, onClose, wsProduct, materialOptions, defaultMaterialSelections, selectedCabinet, onDimensionsChange, onMaterialChange, loading, error, threeJsGDs, onOverhangDoorToggle, onShelfCountChange, onDrawerHeightChange, onDrawerQuantityChange }) => {
  ...
  useEffect(() => {
    ...
    console.log('cabinetKey', cabinetId)
    const entries = Object.entries(wsProduct.dims)
  ...
  const applyPrimaryDimsTo3D = (vals: Record<string, number | string>, changedId?: string) => {
    ...
    //   console.log("onDrawerHeightChange detected", selectedCabinet.carcass?.config?.drawerEnabled, drawerQty)
    // }

    // Apply drawer heights only if enabled and callback present
    if (drawerQty && onDrawerHeightChange) {
      // const drawerQty = drawerQty || 0
              ...
              <div className="flex items-center justify-between mb-2.5 text-gray-700 font-medium">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    for (let i = 0; i < 5; i++) selectedCabinet?.carcass?.config.changedDrawerIndices?.delete(i)
                  }}
              ...
              <div className="space-y-3">
                {dimsList
                  ...
                  .map(([id, dimObj]) => {
                    ...
                    // const isChanged = currentVal !== defVal

                    const drawerQty = selectedCabinet?.carcass?.config?.drawerQuantity || 0
                    const changedIndices = selectedCabinet?.carcass?.config?.changedDrawerIndices
                    const isDependentDrawer = drawerHeightIndex !== null && changedIndices && changedIndices.size >= (drawerQty - 1) && !changedIndices.has(drawerHeightIndex)
                            ...
                            <div className="flex items-center gap-1.5 mb-1.5">
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  applyPrimaryDimsTo3D(next, id)
                                  selectedCabinet?.carcass?.config.changedDrawerIndices?.delete(drawerHeightIndex!)
                                }}
