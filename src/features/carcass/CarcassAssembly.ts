        ...
        if (!approximatelyEqual(heightRatio, 1, 0.0001)) {
          ...
          try {
            ...
            if (productData) {
              ...
              const constraintsMap: Record<number, { min: number; max: number; dimId: string }> = {}
              dimsList.forEach(([dimId, dimObj]) => {
                const gdId = dimObj.GDId
              ...
              constraints = drawerHeights.map((_, index) => constraintsMap[index] || { min: 50, max: this.dimensions.height })
            }
          } catch (e) {
            console.warn("Failed to fetch product data for drawer constraints:", e)
          }

          // Fallback if constraints empty (e.g. product data not found)
          if (constraints.length === 0) {
              constraints = drawerHeights.map(() => ({ min: 50, max: this.dimensions.height }));
          }

          // Validate total min/max height
          const isMinValid = DrawerHeightManager.validateTotalMinHeight(constraints, this.dimensions.height)
          if (!isMinValid) {
          ...
          this.config.drawerHeights = DrawerHeightManager.scaleHeightsProportionally(
            drawerHeights,
            totalCurrentHeight, 
            this.dimensions.height,
            constraints
