...
import { createEndPanelGeometry } from '../utils/geometryUtils';
...
export class CarcassEnd {
  ...
  constructor(props: CarcassEndProps) {
    ...
    // Create geometry for end panel using utility
    const geometry = createEndPanelGeometry({
  ...
  public updateDimensions(height: number, depth: number, thickness: number): void {
    ...
    const newGeometry = createEndPanelGeometry({
      thickness: this.thickness,
      height: this.height,
      depth: this.depth
    });
