...
// Default height from floor for standalone benchtops
const DEFAULT_BENCHTOP_HEIGHT_FROM_FLOOR = 740
...
export const useProductDrivenCreation = ({
  ...
  useEffect(() => {
    ...
    // For standalone benchtops, pass additional properties for positioning and lock icons
    const additionalProps = cabinetType === "benchtop" ? {
      hideLockIcons: false,  // Show lock icons for standalone benchtops
      leftLock: true,        // Default: left locked
      rightLock: false,      // Default: right unlocked (extend to right)
      benchtopHeightFromFloor: DEFAULT_BENCHTOP_HEIGHT_FROM_FLOOR,
    } : undefined
    ...
    if (cabinetData) {
      // Position standalone benchtop at default height from floor
      if (cabinetType === "benchtop") {
        cabinetData.group.position.y = DEFAULT_BENCHTOP_HEIGHT_FROM_FLOOR
      }
      setSelectedCabinetRef.current(cabinetData)
    }
  }, [
