...
export const UnderPanelsModal: React.FC<Props> = ({ isOpen, onClose, wsProducts, onProductSelect }) => {
  const [hoveredProduct, setHoveredProduct] = useState<{ id: string; img: string; name: string } | null>(null)
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 })
  const modalRef = React.useRef<HTMLDivElement>(null)

  // Find all underPanel products by checking design.type3D === 'underPanel'
  const allProducts = React.useMemo(() => {
    if (!wsProducts) return []

    // Get all active products where design.type3D === 'underPanel'
          ...
          >
            <div ref={modalRef} className="bg-white rounded-lg shadow-2xl max-w-[460px] w-full max-h-[80vh] overflow-hidden flex flex-col">
              ...
              <div className="p-3 border-b border-gray-200 flex items-center justify-between bg-gray-50">
                <div>
                  <h2 className="text-xl font-bold text-gray-800">Under Panels</h2>
                </div>
                <button
                  onClick={onClose}
              ...
              <div className="p-3 overflow-y-auto flex-1">
                <div className="rounded-md border border-gray-200 bg-white">
                  {allProducts.length === 0 ? (
                    <div className="p-2 text-xs text-gray-500">No under panel products found</div>
                  ) : (
